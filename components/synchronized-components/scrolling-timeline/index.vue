<!--
  Component Name: Synchronized Scrolling Timeline

  Description:
  This component is a wrapper designed to be used with scroll-based animations. It provides
  functionality to track the progress of an element's scroll position within a specified range.
  The component allows you to define a custom tag for the outer element and set start and end
  offsets for calculating the progress. The component uses the "useScrollingElement" and "useTimeline" hooks to manage
  the progress and integrate with animations.
  ...
  Usage:
  The ScrollingTimeline is used by passing a slot with elements that need to be animated based
  on scroll progress. The "addToTimeline" prop is automatically provided to the slot, which you can
  use to add animation elements to the timeline created by the "useTimeline" hook.
  ...
  <ScrollingTimeline
    ...
    @update:progress="handleProgressUpdate">
		...
		from='{x: 100}'
		to='{x: 0}'
		:addToTimeline="addToTimeline">Animated content</s-tween>
  </ScrollingTimeline>
-->
...
<script setup>
  ...
  const { addToTimeline } = useScrollingTimeline({
    el,
